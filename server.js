...
app.post('/register', (req, res) => {
  const { username, email, password, phone_number, birthdate, socialnumber } = req.body;
...
app.post('/buy-course', verifyToken, (req, res) => {
  ...
  db.query('INSERT INTO user_courses (user_id, course_id) VALUES (?, ?)', [req.userId, courseId], (err, result) => {
    if (err) return res.status(500).send(err);
